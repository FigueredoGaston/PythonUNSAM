...
class Animal(object):
    ...
    def tiene_hambre(self):
        ...
        return True if self.energia < self.energia_maxima else False
        # pass
    ...
    def tener_cria(self):
        """Acá se puede poner comportamiento que sucede al
        tener cria para evitar que tengamás de una cria por ciclo, etc"""
...
class Leon(Animal):
    ...
    def alimentarse(self, animales_vecinos):
        ...
        if self.tiene_hambre():  # no está lleno
            presas_cercanas = [(pos, animal) for (
                pos, animal) in animales_vecinos if animal.es_antilope()]
